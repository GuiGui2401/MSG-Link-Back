...
@section('title', 'Nouveau package sponsoring')
@section('header', 'Créer un package sponsoring')

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form action="{{ route('admin.sponsorship-packages.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Optionnel. Si vide, on utilise seulement l'audience min.</p>
                    @error('reach_max')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number"
                           ...
                           required>
                    <p class="mt-1 text-xs text-gray-500">Pendant combien de jours on doit toucher la cible.</p>
                    @error('duration_days')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
